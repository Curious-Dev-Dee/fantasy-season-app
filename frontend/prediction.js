...
async function handleDailyStreak(userId) {
  ...
  // If no row, you should ensure user_profiles row is created at signup.
...
async function fetchNextMatch() {
  ...
  if (lastFinishedMatch) {
    document
      .getElementById("recentResultContainer")
...
async function renderSelectionView(match) {
  document.getElementById("predictionSubtext").textContent =
    `Next: ${match.team_a.short_code} vs ${match.team_b.short_code}`;

  winnerToggle.innerHTML = `
    <button class="team-option" data-id="${match.team_a.id}">
      ${match.team_a.short_code}
    </button>
    <button class="team-option" data-id="${match.team_b.id}">
  ...
  return `
    <div class="result-item">
      <div class="result-row">
        <label>${label}</label>
        <span class="winner-val">${val}</span>
      </div>
      <div class="pct-bar-bg">
        <div class="pct-bar-fill" style="width:${pct || 0}%"></div>
      </div>
      <div class="pct-label">
        ${pct || 0}% correct (${votes || 0} votes)
      </div>
    </div>
  `;
}
...
submitBtn.onclick = async () => {
  ...
  if (!selectedWinnerId || !mvpId || !topUserId) {
    alert("Please complete your guesses!");
    return;
  }
...
async function checkExistingPrediction() {
  ...
  if (data) {
    ...
    const btn = winnerToggle.querySelector(
      `[data-id="${data.predicted_winner_id}"]`
    );
...
function setupLeagueUIListeners() {
  createLeagueBtn.onclick = async () => {
    ...
    const { data: league, error } = await supabase
      .from("leagues")
      .insert([{ name, invite_code: inviteCode, owner_id: currentUserId }])
      .select()
      .single();
    ...
    await supabase
      .from("league_members")
      .insert([{ league_id: league.id, user_id: currentUserId }]);
  ...
  joinLeagueBtn.onclick = async () => {
    ...
    if (error) {
      alert("You're already in this league or it failed.");
      return;
    }
...
function renderMessage(msg) {
  ...
  div.innerHTML = `
    <span class="msg-user">
      ${msg.user_profiles?.team_name || "Expert"}
    </span>
  ...
  tbody.innerHTML = data
    .map(
      ...
      <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
        ...
        <td style="text-align:right; color:#9AE000; font-weight:800;">
          ${row.total_points}
        </td>
      </tr>`
    )
...
async function fetchUserPredictionPoints() {
  ...
  const total =
    data?.reduce((acc, curr) => acc + (curr.points_earned || 0), 0) || 0;
...
async function fetchExpertsList() {
  ...
  userPredictSelect.innerHTML =
    `<option value="">Select an Expert...</option>` +
    (data || [])
      .map((e) => {
        const label =
          e.user_id === currentUserId ? `${e.team_name} (Me)` : e.team_name;
