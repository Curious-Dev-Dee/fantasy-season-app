...
function render() {
    ...
    // --- 1. IPL CATEGORY & TEAM LIMIT COUNTS ---
    const overseasCount = state.selectedPlayers.filter(p => p.category === 'overseas').length;
    const uncappedCount = state.selectedPlayers.filter(p => p.category === 'uncapped').length;
    
    const teamCounts = {};
    state.selectedPlayers.forEach(p => {
        teamCounts[p.real_team_id] = (teamCounts[p.real_team_id] || 0) + 1;
    });
    const maxFromOneTeam = Math.max(0, ...Object.values(teamCounts));

    // --- 2. THE UNCAPPED DISCOUNT MATH ---
    let subsUsed = 0;
    if (!isResetMatch && state.lockedPlayerIds.length > 0) {
        // Players currently in the XI who were NOT in the last locked XI
        const playersAddedInDraft = state.selectedPlayers.filter(p => !state.lockedPlayerIds.includes(p.id));
        const totalNewPlayers = playersAddedInDraft.length;

        // If any added player is uncapped, 1 sub is FREE
        const hasUncappedAdded = playersAddedInDraft.some(p => p.category === 'uncapped');
        subsUsed = hasUncappedAdded ? Math.max(0, totalNewPlayers - 1) : totalNewPlayers;
    }
    ...
    // Role Count UI
    const roles = { WK: state.selectedPlayers.filter(p => p.role === "WK").length, BAT: state.selectedPlayers.filter(p => p.role === "BAT").length, AR: state.selectedPlayers.filter(p => p.role === "AR").length, BOWL: state.selectedPlayers.filter(p => p.role === "BOWL").length };
    ["WK", "BAT", "AR", "BOWL"].forEach(r => document.getElementById(`count-${r}`).innerText = roles[r] || "");
    
    // --- 4. IPL STRATEGIC VALIDATION ---
    ...
    const isUncappedValid = uncappedCount >= 1;
    const isTeamLimitValid = maxFromOneTeam <= 6;

    const isValid = count === 11 && state.captainId && state.viceCaptainId && 
                    !isOverLimit && hasRequiredRoles && currentCredits <= 100 && 
                    isOverseasValid && isUncappedValid && isTeamLimitValid;

    const saveBtn = document.getElementById("saveTeamBtn");
    saveBtn.disabled = !isValid;

    // Strategic Save Button Text
    if (state.saving) saveBtn.innerText = "SAVING...";
    else if (count < 11) saveBtn.innerText = `ADD ${11 - count} MORE PLAYERS`;
    else if (overseasCount > 4) saveBtn.innerText = `OVERSEAS LIMIT: ${overseasCount}/4`;
    else if (uncappedCount < 1) saveBtn.innerText = "MIN 1 UNCAPPED REQUIRED";
    else if (maxFromOneTeam > 6) saveBtn.innerText = "MAX 6 PER TEAM";
    else if (currentCredits > 100) saveBtn.innerText = `OVER BUDGET: ${(currentCredits-100).toFixed(1)} Cr`;
    else if (!hasRequiredRoles) saveBtn.innerText = "1 WK, 3 BAT, 1 AR, 3 BOWL REQ";
...
function renderList(containerId, list, isMyXi) {
    ...
    container.innerHTML = list.map(p => {
        ...
        // Dynamic Category Badge Logic
        let catBadge = "";
        if (p.category === 'overseas') catBadge = `<span class="badge-cat badge-os">OS</span>`;
        else if (p.category === 'uncapped') catBadge = `<span class="badge-cat badge-uc">UNCAPPED</span>`;
...
function initFilters() {
    ...
    document.getElementById("teamListContent").innerHTML = teams.map(t => `
    ...
    document.getElementById("creditListContent").innerHTML = credits.map(c => `
        <label class="filter-item">
            <span>${c} Cr</span>
