...
let state = { 
    allPlayers: [], selectedPlayers: [], lockedPlayerIds: [],
    baseSubsRemaining: 150, matches: [],
    captainId: null, viceCaptainId: null, s8BoosterUsed: false, boosterActiveInDraft: false,
    currentMatchNumber: 0, lastLockedMatchNumber: 0,
...
function render() {
    ...
    const isValid = count === 11 && state.captainId && state.viceCaptainId && !isOverLimit && roles.WK >= 1 && roles.BAT >= 3 && roles.AR >= 1 && roles.BOWL >= 3;
    const saveBtn = document.getElementById("saveTeamBtn");
    saveBtn.disabled = !isValid;
    saveBtn.innerText = state.saving ? "SAVING..." : (isOverLimit ? "OUT OF SUBS" : "SAVE TEAM");
}
...
function renderList(containerId, list, isMyXi) {
    ...
    container.innerHTML = list.map(p => {
        ...
        let actions = isMyXi ? `
            ...
            `<button class="action-btn-circle ${isSelected ? 'remove' : 'add'}" onclick="togglePlayer('${p.id}')">${isSelected ? '−' : '+'}</button>`;

        return `<div class="player-card">
            <div class="avatar-container"><img src="${photoUrl}" class="player-avatar"></div>
            <div class="player-info"><strong>${p.name}</strong><span>${p.role} • ${getTeamCode(p)} • ${p.credit} Cr</span></div>
            ${actions}
        </div>`;
    }).join('');
}
...
function initFilters() {
    // Teams checkbox list (simplified)
    const teams = [...new Set(state.allPlayers.map(p => p.real_team_id))];
    const teamMenu = document.getElementById("teamMenu");
    if (teamMenu) teamMenu.innerHTML = teams.map(tId => {
        const p = state.allPlayers.find(pl => pl.real_team_id === tId);
        return `<label class="filter-item">${p.team_short_code} <input type="checkbox" onchange="toggleFilter('teams', '${tId}', this)"></label>`;
    }).join('');
}
