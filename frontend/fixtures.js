...
document.addEventListener("click", (e) => {
  ...
  const isTeamClick =
    teamBtn.contains(e.target) ||
    teamPanel.contains(e.target);
...
function renderMatches() {
  ...
  if (!selectedStatuses.has("all")) {
    filtered = filtered.filter(match =>
      selectedStatuses.has(match.status)
    );
  }
  ...
  if (selectedTeams.size > 0) {
    filtered = filtered.filter(match =>
      selectedTeams.has(match.team_a_id) ||
      selectedTeams.has(match.team_b_id)
    );
  }
  ...
  filtered.forEach(match => {
    ...
    const readableDate = new Date(match.start_time)
